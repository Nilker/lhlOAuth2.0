...
namespace Owin.Security.Lhl
{
    ...
    internal class LHLAuthenticationHandler : AuthenticationHandler<LHLAuthenticationOptions>
    {
        ...
        /// </summary>
        /// <returns>
        /// 若返回 false，则公共代码将按顺序调用下一个中间件。若返回 true，则公共代码将开始异步完成过程，而不调用中间件管道的其余部分。
        /// </returns>
        ...
        /// <summary>
        /// 必须由处理程序提供的核心身份验证逻辑。针对每个请求最多调用一次。不要直接调用，而应调用包装 Authenticate 方法。
        /// </summary>
        /// 
        /// <returns>
        /// 由身份验证逻辑提供的票证数据
        ...
        /// <summary>
        /// 如果相关身份验证方案将身份验证交互作为其请求流的一部分处理，则重写此方法以处理 401 质询问题。（例如添加响应标头，或将 401 结果更改为登录页的 302 或外部登录位置。）
        /// </summary>
